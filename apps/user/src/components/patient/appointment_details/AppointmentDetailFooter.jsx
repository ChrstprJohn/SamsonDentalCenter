...
const AppointmentDetailFooter = ({ isCancellable, isReschedulable, isPending, isApproved, hasRescheduled, onCancelClick, onRescheduleClick }) => {
    ...
    return (
        <div className='fixed bottom-0 left-0 right-0 sm:relative z-20 px-6 py-4 border-t border-gray-100 dark:border-gray-800 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md sm:shadow-none'>
            <div className='flex items-center gap-3 w-full justify-between'>
                <div className='hidden sm:block sm:flex-1'></div>
                <div className='flex flex-1 sm:flex-none gap-3 sm:justify-end w-full'>
                    <button
                        onClick={onCancelClick}
                        className='flex-1 sm:flex-none sm:min-w-[180px] inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:py-3 bg-white dark:bg-error-500/10 text-error-600 dark:text-error-500 text-[13px] sm:text-[15px] font-black rounded-lg hover:bg-error-50 dark:hover:bg-error-500/20 transition-[background-color,border-color] border border-error-100 dark:border-error-500/20'
                    >
                    ...
                    {isReschedulable && (
                        <button
                            ...
                            className={`flex-1 sm:flex-none sm:min-w-[160px] inline-flex items-center justify-center gap-2 px-4 py-2.5 sm:py-3 text-[13px] sm:text-[15px] font-black rounded-lg transition-[background-color,box-shadow] ${
                                hasRescheduled
