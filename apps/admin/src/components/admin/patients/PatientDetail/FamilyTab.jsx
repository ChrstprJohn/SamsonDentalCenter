                    ...
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4'>
                        {dependents.map(dep => (
                            <div 
                                ...
                                className='p-3 sm:p-4 border border-gray-200 dark:border-white/5 rounded-2xl hover:border-brand-500/50 transition-all cursor-pointer bg-gray-50/20 dark:bg-white/[0.01] group flex items-center gap-4'
                            >
                                <div className='w-10 h-10 sm:w-12 sm:h-12 rounded-xl border border-gray-200 dark:border-white/5 bg-white dark:bg-white/5 flex items-center justify-center shrink-0 shadow-sm'>
                                    {dep.avatar_url ? (
                                        <img src={dep.avatar_url} alt={dep.full_name} className="w-full h-full object-cover rounded-xl" />
                                    ) : (
                                        <div className='w-full h-full flex items-center justify-center bg-gradient-to-br from-brand-400 to-brand-600 text-white font-bold text-base sm:text-lg rounded-xl'>
