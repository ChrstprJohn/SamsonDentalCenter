...
const PatientSelectStep = ({ primaryPatient, dependents, selectedPatientId, onSelect, onNext, onPrev }) => {
    ...
    return (
        <div className='space-y-6'>
            <div className='mb-6'>
                <h3 className='text-xs font-medium text-gray-400 capitalize mb-4 flex items-center gap-2'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                    {allOptions.map((option) => (
                        ...
                        >
                            ...
                            <div className='grow'>
                                <p className='text-sm font-bold text-gray-900 dark:text-white capitalize group-hover:text-brand-500 transition-colors'>
                                    {option.name}
                                </p>
                                <span className='px-1.5 py-0.5 rounded bg-gray-100 dark:bg-white/5 text-gray-400 text-[11px] font-medium capitalize tracking-tighter mt-1 inline-block'>
                                    {option.relationship}
                                </span>
                            </div>
