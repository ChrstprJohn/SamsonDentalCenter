...
import { User, Clock, Phone, ClipboardList, StickyNote, CircleCheckBig, ChevronLeft, X } from 'lucide-react';

const CheckoutView = ({ appointment, patient, onBack, onConfirm }) => {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">
            ...
            <div className="px-4 sm:px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm z-30 shrink-0">
                <button 
                    onClick={onBack}
                    className="p-2 rounded-xl bg-gray-100 dark:bg-white/[0.05] text-gray-500 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-white/[0.1] transition-all shadow-theme-xs active:scale-95"
                >
                    <ChevronLeft size={20} />
                </button>
                <div className="flex flex-col items-center text-center">
                    <h2 className="text-xl font-bold text-gray-900 dark:text-white font-outfit leading-tight">Finalize Check Out</h2>
                    <p className="text-[12px] sm:text-xs text-gray-500 dark:text-gray-400 font-medium mt-0.5">Review treatment details and patient information.</p>
                </div>
            ...
            <div className="grow overflow-y-auto no-scrollbar">
                <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-[640px] mx-auto">
                    ...
                    <div className="rounded-2xl border border-gray-100 dark:border-white/5 bg-gray-50/30 dark:bg-white/[0.01] overflow-hidden shadow-sm">
                        <div className="p-5 flex items-start justify-between gap-4 border-b border-gray-50 dark:border-white/5">
                            ...
                            <div className="text-right shrink-0">
                                ...
                                <p className="text-sm font-bold text-gray-900 dark:text-white font-outfit capitalize">{appointment.doctor}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 divide-x divide-gray-50 dark:divide-white/5 bg-white dark:bg-gray-800/20">
                            <div className="p-4 flex flex-col gap-1">
                                ...
                                <span className="text-sm font-bold text-[#0B1120] dark:text-white font-outfit">{appointment.startTime}</span>
                            </div>
                            <div className="p-4 flex flex-col gap-1">
                                <span className="text-[12px] font-bold text-gray-400 dark:text-gray-500 capitalize flex items-center gap-1.5">
                                    <Clock size={10} /> End
                                </span>
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400 font-outfit">{appointment.endTime}</span>
                            </div>
                            <div className="p-4 flex flex-col gap-1 col-span-2 sm:col-span-1 border-t sm:border-t-0 border-gray-50 dark:border-white/5">
                                ...
                                <span className="text-sm font-medium text-emerald-600 dark:text-emerald-400 font-mono">{appointment.phone || '+63 920 987 6543'}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-brand-50/20 dark:bg-brand-500/[0.02] border-t border-gray-50 dark:border-white/5 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[12px] font-medium capitalize px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 dark:bg-amber-900/30">In Progress</span>
                            </div>
                        </div>
                    </div>
                                ...
                                >
                                    ...
                                    <div className="min-w-0">
                                        <div className="flex flex-col">
                                            ...
                                            <span className={`text-[11px] font-medium capitalize  ${proc.type === 'Specialized' ? 'text-amber-500' : 'text-brand-500/70'}`}>
                                                {proc.type}
                                            </span>
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-3 pb-2">
                        ...
                        <div className="p-4 bg-amber-50/40 dark:bg-amber-500/[0.02] border border-amber-100/30 dark:border-amber-500/10 rounded-2xl shadow-inner">
                            <p className="text-[14px] text-gray-600 dark:text-gray-400 leading-relaxed italic font-medium">
                                "{appointment.notes || 'Procedure completed without complications. Patient advised to return in 2 weeks for the final crown placement. Prescribed Ibuprofen 400mg for post-op discomfort.'}"
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 pt-4 pb-12">
                        <button 
                            onClick={onBack}
                            className="inline-flex items-center justify-center gap-2 rounded-xl transition flex-1 h-12 font-medium text-[11px] capitalize border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/[0.03] shadow-theme-xs active:scale-95"
                        >
                            Cancel
                        </button>
                        <button 
                            onClick={onConfirm}
                            className="inline-flex items-center justify-center gap-2 rounded-xl transition flex-1 h-12 font-medium text-[11px] capitalize bg-brand-500 text-white shadow-xl shadow-brand-500/20 hover:bg-brand-600 active:scale-95"
                        >
