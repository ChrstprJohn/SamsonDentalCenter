        ...
        <div className='flex-1 flex flex-col space-y-6'>
            ...
            <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center justify-between mb-8 px-1">
                    <h3 className="text-xs font-bold capitalize text-gray-500 dark:text-gray-400 flex items-center gap-2">
                        <User className="size-4" />
                        Dentist Availability
                    </h3>
                    <div className="flex flex-col items-end">
                        <span className="text-[12px] font-medium capitalize text-gray-400 mb-1">Assigned Doctor</span>
                        <span className="text-xs font-bold text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-500/10 px-3 py-1.5 border border-brand-100 dark:border-brand-500/20 shadow-sm rounded-xl">{dentist}</span>
                    </div>
                </div>
                ...
                <div className="relative h-16 bg-gray-50 dark:bg-gray-900/50 rounded-2xl flex items-center overflow-visible border border-gray-200 dark:border-gray-700 shadow-inner px-6 mx-2 mb-10">
                    ...
                    {busySlots.map((pos, idx) => (
                        <div 
                            ...
                            style={{ left: `${pos}%` }}
                        >
                            <span className="text-[11px] text-gray-400 font-medium capitalize">Busy</span>
                        </div>
                    ))}
                    ...
                    {slotPosition >= 0 && slotPosition <= 95 && (
                        ...
                        >
                            <span className="text-[11px] text-brand-600 dark:text-brand-400 font-bold capitalize tracking-tighter">Req</span>
                            <span className="text-xs font-medium text-brand-700 dark:text-brand-300 mt-0.5">{timeStr}</span>
                        </div>
                    )}
                    ...
                    <div className="absolute -bottom-6 left-0 w-full flex justify-between px-2">
                        {[9,11,1,3,5].map(h => (
                            <span key={h} className="text-[12px] font-bold text-gray-400 tracking-tighter">
                                {h}:00{h > 8 && h < 12 ? 'AM' : 'PM'}
                            </span>
                        ))}
                    </div>
                ...
                {busySlots.some(pos => Math.abs(pos - slotPosition) < 8) ? (
                    <p className="text-sm text-error-600 dark:text-error-400 text-center bg-error-50 dark:bg-error-500/10 py-3 rounded-xl border border-error-100 dark:border-error-500/20 font-bold animate-pulse">
                        ⚠️ Conflict detected! Dentist is busy at this time.
                    </p>
                ) : (
                    <p className="text-sm text-success-600 dark:text-success-500 text-center bg-success-50 dark:bg-success-500/10 py-3 rounded-xl border border-success-100 dark:border-success-500/20 font-medium">
