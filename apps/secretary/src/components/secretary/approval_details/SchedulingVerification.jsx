            ...
            <div className='w-full space-y-5 sm:space-y-6'>
                ...
                <div>
                    <div className='flex items-center justify-between mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-gray-100 dark:border-white/5'>
                        <h3 className='text-[11px] font-medium text-gray-400 dark:text-gray-500 capitalize'>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-[12px] font-medium capitalize tracking-tighter ${isConflict ? 'text-error-500' : 'text-success-500'}`}>
                ...
                <div className={`flex items-start gap-3 sm:gap-4 p-4 sm:p-5 rounded-2xl border transition-all duration-500 ${
                    ...
                    <div className="space-y-1">
                        <span className={`block text-[12px] font-medium capitalize  ${isConflict ? 'text-error-700/60 dark:text-error-500/60' : 'text-success-700/60 dark:text-success-500/60'}`}>
                ...
                <div className="pt-2 pb-6">
                    <div className="relative h-14 bg-gray-50/50 dark:bg-gray-950 rounded-2xl flex items-center overflow-visible border border-gray-100 dark:border-gray-800/50 shadow-inner px-6">
                        {busySlots.map((pos, idx) => {
                            ...
                            return (
                                <div 
                                    ...
                                    style={{ left: `${pos}%` }}
                                >
                                    <span className={`text-[6px] font-medium capitalize tracking-tighter hidden sm:block ${isSlotConflict ? 'text-error-600 dark:text-error-400' : 'text-gray-400'}`}>
                        ...
                        {slotPosition >= 0 && slotPosition <= 95 && (
                            ...
                            >
                                <span className="text-[11px] font-medium tabular-nums tracking-tighter leading-none">{timeStr}</span>
                                <span className="text-[6px] font-medium capitalize opacity-60 mt-0.5 hidden sm:block">Req</span>
                            </div>
                        )}
                        <div className="absolute -bottom-5 left-0 w-full flex justify-between px-4">
                            {[9,11,1,3,5].map(h => (
                                <span key={h} className="text-[11px] font-bold text-gray-300 dark:text-gray-600 tracking-tighter tabular-nums capitalize">
                                    {h}{h > 8 && h < 12 ? 'am' : 'pm'}
                                </span>
                            ))}
                        </div>
