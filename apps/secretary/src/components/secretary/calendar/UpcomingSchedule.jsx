        ...
        <div className="flex flex-col border border-gray-300 dark:border-gray-700 rounded-xl bg-white dark:bg-white/[0.03]">
            <div className="p-5 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    <button 
                        onClick={onBlockTime}
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-xl transition text-[11px] sm:text-sm font-bold h-9 sm:h-10 px-3 sm:px-5 bg-red-50 text-red-700 hover:bg-red-100 dark:bg-red-500/10 dark:text-red-400 dark:hover:bg-red-500/20 border border-transparent dark:border-red-900/30"
                    >
                ...
                <div className="grid border-b border-gray-300 dark:border-gray-700 bg-gray-50/20 dark:bg-transparent" style={{ gridTemplateColumns: "var(--gutter-width) repeat(7, 1fr)" }}>
                    ...
                    {currentWeekDates.map((date, i) => (
                        <div key={i} className={`flex flex-col items-center sm:items-start p-1 sm:p-3 border-r border-gray-300 dark:border-gray-700 last:border-r-0 ${i === 0 ? 'bg-brand-50/30 dark:bg-brand-500/5' : ''}`}>
                            <span className={`text-[11px] sm:text-lg font-medium ${i === 0 ? 'text-brand-500' : 'text-gray-900 dark:text-white'}`}>{date.getDate()}</span>
                            <span className={`text-[11px] sm:text-[12px] font-bold capitalize  mt-0.5 ${i === 0 ? 'text-brand-500 opacity-80' : 'text-gray-400'}`}>{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][date.getDay()]}</span>
                        </div>
                    ))}
                </div>
                        ...
                        <div className="sticky left-0 z-20 flex-shrink-0 border-r border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900/90 backdrop-blur-sm" style={{ width: "var(--gutter-width)" }}>
                            {Array.from({ length: 21 }).map((_, i) => {
                                ...
                                return (
                                    <span 
                                        key={i} 
                                        className={`absolute left-0 right-0 text-center text-[11px] sm:text-[13px] font-medium tabular-nums -translate-y-1/2 select-none ${isHour ? 'text-gray-800 dark:text-gray-100' : 'text-gray-400'}`} 
                                ...
                                return (
                                    <div key={colIndex} className="relative border-r border-gray-300 dark:border-gray-700 last:border-r-0 bg-white dark:bg-transparent">
                                        ...
                                        {dayAppointments.map(app => {
                                            ...
                                            return (
                                                <div key={app.id} className="absolute left-1 right-1 rounded-xl border-l-4 border-brand-500 bg-brand-50/80 dark:bg-brand-500/10 p-2 shadow-theme-sm z-10" style={{ top, height }}>
                                                    <div className="font-medium text-brand-700 dark:text-brand-400 capitalize text-[11px] truncate">{app.service_name}</div>
                                                    <div className="font-bold text-gray-900 dark:text-white truncate text-[12px]">{app.patient_name}</div>
                                                </div>
                                            );
                                        })}
                                    </div>
