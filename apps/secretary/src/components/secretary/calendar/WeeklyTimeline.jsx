...
const WeeklyTimeline = ({ doctor, events = [], timeBounds = { minStart: 8, maxEnd: 18 }, onBlockClick }) => {
    ...
    const ROW_PX = daysToShow === 3 ? 48 : 64; // Compact 48px on mobile, efficient 64px on desktop
    const SPACER_PX = 16;
            ...
            <div className="flex flex-col h-auto overflow-hidden" style={{ '--gutter-width': 'clamp(56px, 15vw, 90px)' }}>
                ...
                <div className='flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.01] gap-4'>
                    ...
                    <div className="flex items-center gap-3">
                        <Button variant="outline" size="sm" onClick={goToday} className="text-[12px] font-medium capitalize px-4 h-8 border-gray-200 dark:border-white/5 rounded-xl">Today</Button>
                        <div className="flex items-center gap-2">
                            <button onClick={() => nav(-7)} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-white dark:bg-white/[0.03]/5 border border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400 transition-all">
                                <ChevronLeft size={16} />
                            </button>
                            <button onClick={() => nav(7)} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-gray-100 dark:hover:bg-white dark:bg-white/[0.03]/5 border border-gray-200 dark:border-gray-800 text-gray-500 dark:text-gray-400 transition-all">
