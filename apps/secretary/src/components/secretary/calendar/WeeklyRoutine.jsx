            ...
            <div className="p-5 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                    ...
                    <button 
                        ...
                        className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 rounded-xl transition text-[11px] sm:text-sm font-bold h-9 sm:h-10 px-3 sm:px-5 bg-white text-gray-700 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03] dark:hover:text-gray-300"
                    >
            ...
            <div className="overflow-hidden bg-white dark:bg-transparent">
                ...
                <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-transparent">
                    {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                        <div key={day} className="py-3 text-center text-[12px] font-bold capitalize text-gray-400">{day}</div>
                    ))}
                </div>
                    ...
                    {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        return (
                            <div key={day} className={`relative aspect-square p-1.5 sm:p-3 flex flex-col border-r border-b border-gray-200 dark:border-gray-700 ${isToday ? 'bg-brand-50/20 dark:bg-brand-500/5' : 'bg-white dark:bg-transparent'}`}>
                                <span className={`text-xs sm:text-lg font-medium ${isToday ? 'text-brand-500' : 'text-gray-400 dark:text-gray-500'}`}>{day}</span>
                                <div className="mt-auto">
                                    <span className={`text-[7px] sm:text-[12px] font-bold capitalize  truncate ${
