...
const AppointmentHistoryPage = () => {
    ...
    const [statusFilter, setStatusFilter] = useState('history'); // 'history' = COMPLETED, CANCELLED, NO_SHOW
    ...
    const getStatusStyle = (status) => {
        ...
        switch (s) {
            ...
            case 'CANCELLED': 
            case 'LATE_CANCEL': return { color: 'error', label: 'Cancelled' };
            case 'NO_SHOW': return { color: 'error', label: 'Missed' };
    ...
    const breadcrumbTitle = selectedAppointment ? "History Details" : "Appointment History";

    return (
        <div className="flex flex-col min-h-[calc(100vh-140px)] w-full overflow-x-hidden pb-8">
            <PageBreadcrumb 
                pageTitle={breadcrumbTitle} 
                parentName={selectedAppointment ? "Appointment History" : null}
                parentPath={selectedAppointment ? "/history" : null}
            />
                        ...
                        <div className='border-b border-gray-200 dark:border-gray-700/60 bg-white dark:bg-transparent'>
                            ...
                            <div className='px-4 sm:px-6 pb-5 pt-2'>
                                <div className='flex flex-nowrap items-center gap-3 overflow-x-auto no-scrollbar py-2'>
                                    ...
                                    <div className='relative w-[170px] sm:w-[200px] shrink-0'>
                                        ...
                                        >
                                            ...
                                            <option value="NO_SHOW">Missed (No Show)</option>
                                        </select>
                                ...
                                appointments.map((apt) => {
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className='group relative bg-white dark:bg-white/[0.03] sm:rounded-xl border-b sm:border border-gray-100 dark:border-gray-800 sm:shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer overflow-hidden flex flex-row items-center'
                                        >
                                            ...
                                            <div className='flex-grow flex items-center min-w-0'>
                                                <div className='flex sm:hidden gap-4 w-full pl-6 pr-4 py-4 items-center'>
                                                    <div className='shrink-0'>
                                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-lg ${badgeColor === 'error' ? 'bg-error-500' : badgeColor === 'warning' ? 'bg-warning-500' : badgeColor === 'success' ? 'bg-success-500' : 'bg-brand-500'}`}>
                                                ...
                                                <div className='hidden sm:flex flex-grow px-8 py-5 items-center gap-8 min-w-0'>
                                                    <div className='shrink-0'>
                                                        <div className={`w-14 h-14 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg ${badgeColor === 'error' ? 'bg-error-500' : badgeColor === 'warning' ? 'bg-warning-500' : badgeColor === 'success' ? 'bg-success-500' : 'bg-brand-500'}`}>
                                                            {getInitial(patientName)}
                                                        </div>
                                                    </div>
